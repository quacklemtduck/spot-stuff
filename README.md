...
- Throughout the repository, the name `spot_stuff` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of spot_stuff (in files/directories) to your_fancy_extension_name
...
We provide an example UI extension that will load upon enabling your extension defined in `source/spot_stuff/spot_stuff/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/spot_stuff"
    ]
}
```
